...
class Movie extends React.Component{
  ...
  remove=(e)=>{
    var movs=this.state.movies; var id=e.target.id;    
    let rem = () => {
      return movs.filter((el) =>
        el.id != id
      );
    }
    //e.target.id
    this.setState({movies: rem()}); console.log(this.state.movies)
  }
  ...
  render() { 
    return(
      <div className='mov'>
        ...
        <div className='main'>
          {this.state.movies.map((cont,i)=><MovieBox key={i} obj={cont}><button id={i} className='remove' onClick={this.remove}>Remove</button></MovieBox>)}      
